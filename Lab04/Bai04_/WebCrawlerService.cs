    ...
    public class WebCrawlerService
    {
        ...
        private async Task<Movie> ExtractBasicMovieInfo(HtmlAgilityPack.HtmlNode node)
        {
            try
            {
                ...
                var posterUrl = imgNode?.GetAttributeValue("src", "") ?? imgNode?.GetAttributeValue("data-src", "");

                if (string.IsNullOrEmpty(title) || title.Length < 2 ||
                    title.Contains("Trang chủ") || title.Contains("Beta") ||
                    title.Contains("Cinema") || title == "Xem thêm")
                    return null;
